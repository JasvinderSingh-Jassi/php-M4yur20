...
### For random comic fetching, I have written a function that takes the url[https://c.xkcd.com/random/comic/] as an input argument then extracts all the HTML code from the webpage of the input url and then after applying filter to get only meta tags description, it returns the description of the meta tag containing the serial number of the comic, from that description data after removing all the unwanted characters we will get only the serial number of the comic. After getting the serial number of a random comic, it is appended to the API link [https://xkcd.com/'.[random_number_here].'/info.0.json] then a get request is made in which JSON format of that comic is returned and using this JSON data, that particular comic is then sent to all the users which are subscribed.
